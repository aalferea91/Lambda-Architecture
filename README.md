...
To be able to capture the number views of each comment we are going to write to a Kafka topic and then with Spark we will update the collection in our mongodb for the views, and also we will store that information in a S3 bucket, in order to do analytical queries to it. We do it this way in order to separate the transactional database from our analytical database so the queries to our database will never affect the transactional layer.
...
Once the notebook is created, we can open it in a  pyspark instance and execute the code in the following jupyternotebook:

https://github.com/aalferea91/Lambda-Architecture/blob/main/Spark%20Streaming.ipynb
